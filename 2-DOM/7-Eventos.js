...
addEventListener
   ...
   *mouseenter   - quando o mouse entra em determinada área
   *mouseout     - quando o mouse sai de determinada área

   *scroll       - quando a página rola

   *load         - quando a página interira terminou de carregar
   *DOMContentLoaded - quando o HTML já foi carregado
   ...
   *Ex: elemento.addEventListener("evento", função)
   ...
   Para que não fique confuso o exemplos abaixo, os exemplos abaixo poderão ser limpos do console com uma tecla.

-Parâmetro (event)
   O event é um objeto que contém todas as informações sobre o evento que ocorreu. Ele é fornecido automaticamente pelo navegador sempre que um evento é disparado. Esse objeto é necessário para que você consiga identificar e acessar detalhes do que aconteceu, como qual tecla foi pressionada ou qual elemento foi afetado.

   Na prática qualquer nome pode ser dado para esse parâmetro, mas é melhor utilizar (event) por boas práticas.
   ...
   *keydown, keyup, key, keypress
   ...
   Os eventos do teclado são eventos do DOM disparados quando o usuário pressiona ou solta uma tecla física do teclado. Eles permitem que o JS reaja imediatamente às ações do usuário, esses eventos são representados pelo objeto #KeyboardEvent.
...
/*
Para evitar que tenha-se que diferenciar um tecla quando é maiúscula ou minúscula, pode-se usar event.code, ela irá retorna a posição da tecla física. Observe que foi utilizado no início desse módulo para limpar o console com a tecla C, observe também que o valor é uma String*/
